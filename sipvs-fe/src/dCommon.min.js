...
(function () {
  var mapException = function (e, isLaunch) {
    if (ditec.dbridge2.isAppException(e)) {
      return ditec.utils.createDitecError(
        new Error(),
        e.code != null ? e.code.asNumber() : null,
        e.message,
        e.toString() + " " + e.remoteStackTrace,
      );
    } else {
  ...
  (ditec.AbstractLauncher2Wrapper.prototype._asLong = function (value) {
    ...
    (ditec.AbstractLauncher2Wrapper.prototype._invokeMethod = function (
      appId,
      methodName,
      args,
      returnType,
...
(function () {
  ...
  var ByteArray = function (value) {
    if (value === null || value === undefined) {
      ...
      if (typeof value === "string") {
        if (value.length % 4 !== 0) {
          throw new Error(
            "Invalid length of BASE64 ByteArray: " + value.length,
          );
        }
  ...
  var createDBridgeException = function (
    error,
    code,
    msg,
    remoteStackTrace,
  ...
  var DataTypeComplex = {
    ...
    DATE_TIME: -5,
    ORIGIN: -6,
    INVOKE_METHOD_REQ: -128,
    INVOKE_METHOD_RESP: -127,
  };
  var connectionIdSeq = 0;
  var WebExtensionsIo = function (
    messageCallback,
    errorCallback,
    closeCallback,
  ) {
  ...
  WebExtensionsIo.prototype.init = function () {
    setTimeout(
      function () {
        this._initPromiseWrapper.reject(
          createDBridgeException(
            new Error(),
            ditec.dbridge2.DB_EXTENSION_NOT_FOUND,
            "D.Launcher 2 web extension not found",
          ),
        );
      }.bind(this),
    ...
    window.postMessage(
      { "$ditec.dbridge2.Msg": "initReq", conn: this._connectionId },
      "*",
    );
  ...
  WebExtensionsIo.prototype.open = function () {
    window.postMessage(
      { "$ditec.dbridge2.Msg": "openReq", conn: this._connectionId },
      "*",
    );
  ...
  WebExtensionsIo.prototype.close = function () {
    window.postMessage(
      { "$ditec.dbridge2.Msg": "closeReq", conn: this._connectionId },
      "*",
    );
  };
          ...
          if ("closeResp" === msg) {
            ...
            if (error) {
              ...
              if (error.startsWith("No such native application")) {
                ...
                if (
                  error.startsWith("Specified native messaging host not found")
                ) {
                  code = ditec.dbridge2.DB_DLAUNCHER2_NOT_FOUND;
                } else {
    ...
    try {
      ...
      this._initPromiseWrapper.reject(
        createDBridgeException(
          e,
          ditec.dbridge2.DB_EXTENSION_NOT_FOUND,
          "Native Messaging communication plugin not found",
        ),
      );
      return this._initPromiseWrapper.promise;
    }
  ...
  DnmcIo.prototype.open = function () {
    ...
    if (this._dnmc.errorCode == -2) {
      this._openPromiseWrapper.reject(
        createDBridgeException(
          new Error(),
          ditec.dbridge2.DB_DLAUNCHER2_NOT_FOUND,
          "D.Launcher not installed",
        ),
      );
    } else {
      if (this._dnmc.errorCode == -3) {
        this._openPromiseWrapper.reject(
          createDBridgeException(
            new Error(),
            ditec.dbridge2.DB_IE_PROTECTED_MODE_ENABLED,
            "IE protected mode is not disabled",
          ),
        );
      } else {
        if (this._dnmc.errorCode != 0) {
          this._openPromiseWrapper.reject(
            createDBridgeException(
              new Error(),
              ditec.dbridge2.DB_GENERAL,
              "D.Dnmc error: " + this._dnmc.errorCode,
            ),
          );
        }
      }
  ...
  UnsupportedIo.prototype.init = function () {
    ...
    initPromiseWrapper.reject(
      createDBridgeException(
        new Error(),
        ditec.dbridge2.DB_PLATFORM_NOT_SUPPORTED,
  ...
  ParserException.prototype.dataRecieved = function (data) {
    if (data[0] === "sk.ditec.dbridge2.AppException") {
      this._value = createAppException(
        null,
        data[3],
        data[1],
        data[2],
      ...
      if (data[0] === "sk.ditec.dbridge2.DBridgeException") {
        this._value = createDBridgeException(
          null,
          data[3],
          data[1],
          data[2],
  ...
  DBridgeClient.prototype._launch = function (uriList, reconnectToken) {
    ...
    var ret = self._io
      ...
      .then(function () {
        ...
        self._sendMessage({
          op: "launch",
          args: { uriList: uriList, reconnectToken: reconnectToken },
          protocolMin: 1,
          protocolMax: 1,
        });
        return p;
  ...
  DBridgeClient.prototype._sendMessage = function (msg) {
    msg["jSeq"] = this._jSeq++;
    if (this._closed) {
      throw createDBridgeException(
        new Error(),
        ditec.dbridge2.DB_CONNECTION_CLOSED,
        "Connection closed",
      );
    }
    this._io.send(msg);
    ...
    if (jSeq == this._expectedJSeq) {
      ...
      if (jSeq > this._expectedJSeq) {
        ...
        if (jSeq === undefined || jSeq === null) {
          ...
          this._close(
            createDBridgeException(new Error(), null, "Unexpected jSeq number"),
          );
        }
      }
    }
                  ...
                  if (data instanceof ditec.dbridge2.ByteArray) {
                    ...
                    if (data instanceof Error) {
                      ...
                      if (Array.isArray(data)) {
                        ...
                        this._serialize(
                          new ditec.dbridge2.Integer(data.length),
                        );
                        ...
                        if (data instanceof Map) {
                          ...
                          this._serialize(
                            new ditec.dbridge2.Integer(data.size),
                          );
                          ...
                          if (data instanceof Date) {
                            ...
                            if (data instanceof ditec.dbridge2.Origin) {
                              ...
                              if (data instanceof InvokeMethodReq) {
                                this._sendMessage({
                                  op: DataTypeComplex.INVOKE_METHOD_REQ,
                                });
                                this._serialize([
                                  new ditec.dbridge2.Integer(data.seq),
                                  data.appId,
                                  data.methodName,
                                  data.args,
                                ]);
  ...
  DBridgeClient.prototype._serializeException = function (s) {
    ...
    if (s.name === "sk.ditec.dbridge2.AppException") {
      l = [
        "sk.ditec.dbridge2.AppException",
        s.message,
        stackTrace,
        s.code,
        s.data,
      ...
      if (s.name === "sk.ditec.dbridge2.DBridgeException") {
        l = [
          "sk.ditec.dbridge2.DBridgeException",
          s.message,
          stackTrace,
          String(s.code),
          s.data,
  ...
  ditec.dbridge2 = {
    DB_SUCCESS: "DB0",
    DB_GENERAL: "DB200",
    DB_CONNECTION_CLOSED: "DB204",
    DB_PLATFORM_NOT_SUPPORTED: "DB205",
    DB_EXTENSION_NOT_FOUND: "DB206",
    DB_DLAUNCHER2_NOT_FOUND: "DB207",
    DB_IE_PROTECTED_MODE_ENABLED: "DB208",
    DB_UNKNOWN_APPLICATION: "DB209",
    DB_UNKNOWN_METHOD: "DB210",
    DB_UNSUPPORTED_PROTOCOL: "DB211",
    DL_SUCCESS: "DL0",
    DL_CANCELLED: "DL63",
    DL_GENERAL_ERROR: "DL62",
    DL_APP_CRASHED: "DL61",
    DL_APP_NOT_AVAIABLE: "DL60",
    DL_APP_ACTION_NOT_AVAIABLE: "DL59",
    DL_JAVA_NOT_FOUND: "DL58",
    Integer: Integer,
    Long: Long,
    Char: Char,
    ByteArray: ByteArray,
    Origin: Origin,
    launch: function (uriList, reconnectToken) {
    ...
    isDBridgeException: function (e) {
      return (
        e instanceof Error && e.name === "sk.ditec.dbridge2.DBridgeException"
      );
    },
    isGenericException: function (e) {
      return (
        e instanceof Error && e.name === "sk.ditec.dbridge2.GenericException"
      );
    },
    log: function (msg, o) {
      try {
